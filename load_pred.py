...
def mean_load_pred(input_data_list):
    for input_data_dict in input_data_list:
        try:
            ...
            df['hour'] = np.array(pd.DatetimeIndex(df['GregorianDate']).hour)

            le_route = LabelEncoder()
            df['FlightRoute'] = le_route.fit_transform(df['FlightRoute'])

            df1 = df.filter(
                ['year', 'month', 'day', 'dayofweek', 'hour', 'FlightRoute', 'is_holiday', 'PaxWeight_shift1',
                 'PaxWeight_shift2', 'PaxWeight'])
            df1 = df1.iloc[100:, :]
